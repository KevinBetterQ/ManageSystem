        ...
        <td height="40" class="font42"><table width="100%" border="0" align="center" cellpadding="4" cellspacing="1" class="newfont03">
          ...
          <tr bgcolor="#EEEEEE">
            ...
            <td>调动原因</td>
             ...
             <%List<ChangeSkdept>changeSkdepts=(List<ChangeSkdept>)request.getAttribute("changedepts");
             
                  if(changeSkdepts==null)
                  {
                  %>
                  <tr><td colspan="7">没有符合条件的数据</td></tr>
                  <%}else{ 
                  	for(ChangeSkdept changeSkdept:changeSkdepts)
                  	{
                   
                  %>
          <tr bgcolor="#FFFFFF">
            <td width="128" height="20"><input type="checkbox" name="delid"/></td>
            <td width="128" ><%=changeSkdept.getOlddept()   %></td>
            <td width="128"><%=changeSkdept.getNewdept()   %></td>
            <td width="128"><%=changeSkdept.getName()   %></td>
            <td width="128"><%=changeSkdept.getSex()   %></td>
            <% String[] time = changeSkdept.getDepttime().split(" "); %>
            <td width="130"><%=time[0] %></td>
            <td><%=changeSkdept.getDeptreason()   %></td>
          </tr>
           <%}} %>
